...
#undef NM_VPN_PLUGIN_ERROR
#define NM_VPN_PLUGIN_ERROR (g_quark_from_static_string("NM_VPN_PLUGIN_ERROR"))
...
{
	g_debug("nm_vpn_plugin_utils_load_editor(): module_name=%s, factory_name=%s", module_name, factory_name);
	static struct {
		gpointer factory;
		void *dl_module;
		char *module_name;
	...
	if (!g_path_is_absolute (module_name)) {
		 ...
		 */
		if (!dladdr((void *)nm_vpn_plugin_utils_load_editor, &plugin_info)) {
			/* Really a "can not happen" scenario. */
			g_warning ("dladdr failed: %s", dlerror ());
			g_set_error (error,
			             NM_VPN_PLUGIN_ERROR,
	...
	if (cached.factory) {
		...
		gpointer factory;
		void *dl_module;
		g_info ("dlopen() module%s", module_path);
	...
	g_debug ("calling editor factory %s", factory_name);
